...
#define LV_DELAY_NO_OOP TICK_INCREMENTATION
...
bool lv_timer_sem_init() {
    ...
    lvgl_timer_mutex = xSemaphoreCreateRecursiveMutex();
...
bool lv_timer_sem_take(void) {
    return (xSemaphoreTakeRecursive(lvgl_timer_mutex, portMAX_DELAY) == pdTRUE);
}

void lv_timer_sem_give(void) {
    if (lvgl_timer_mutex)
        xSemaphoreGiveRecursive(lvgl_timer_mutex);
}
#endif /* #if (LV_TIMER_TASK_METHOD == USE_MUTEX) */
#endif /* #if (LV_TICK_SOURCE == LV_TICK_SOURCE_TASK) */
...
void lv_main_task(void* parameter) {
    ...
    while (true) {
        ...
        vTaskDelayUntil(&tick, pdMS_TO_TICKS(LV_DELAY_NO_OOP));
    }
}
...
void lv_main_tick_task(void* parameter) {
    ...
    while (true) {
        ...
        vTaskDelayUntil(&tick, pdMS_TO_TICKS(LV_DELAY_NO_OOP));  // Delay precis mult mai rapid asa
    }
}
...
void lv_main_tick_task(void* parameter) {
    ...
    while (true) {
        ...
        xTaskDelayUntil(&tick, pdMS_TO_TICKS(LV_DELAY_NO_OOP));  // Delay precis mult mai rapid asa
    }
}
...
#if (LV_TICK_SOURCE == LV_TICK_SOURCE_TIMER) || (LV_TIMER_TASK_METHOD == USE_MUTEX)
    lv_timer_sem_init();
